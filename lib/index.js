...
async function prepareContainers(projectName, gsServiceVersion, devcontainerDir, composeOptions, mongodb, postgresql, mysql) {
...
async function GSUpdate(composeOptions) {
    ...
    try {
        ...
        let { projectName, mongodb, mongoDbName, postgresql, postgresDbName, kafka, elasticsearch, redis, userUID, svcPort, elasticsearchPort, postgresDbPort, zookeeperPort, kafkaPort, redisPort, mongoDb1Port, mongoDb2Port, mongoDb3Port, mysql, mysqlDbName, mysqlDbPort } = gs;
        ...
        try {
            ...
            if (!gs.mysql) {
                mysql = (0, utils_1.ask)("Do you need mysql? [y/n] ");
                if (mysql) {
                    mysqlDbName =
                        (0, utils_1.prompt)("Please enter name of the mysql database [default: test] ") || "test";
                    mysqlDbPort = Number((0, utils_1.prompt)("Please enter host port for mysql [default: 3306] ") ||
                        3306);
                }
            }
            if (!gs.kafka) {
            ...
            await prepareContainers(projectName, gsServiceVersion, devcontainerDir, composeOptions, mongodb, postgresql, mysql);
...
async function GSCreate(projectName, options, composeOptions) {
    ...
    try {
        ...
        const mysql = (0, utils_1.ask)("Do you need mysql? [y/n] ");
        let mysqlDbName, mysqlDbPort;
        if (mysql) {
            mysqlDbName =
                (0, utils_1.prompt)("Please enter name of the mysql database [default: test] ") ||
                    "test";
            mysqlDbPort = Number((0, utils_1.prompt)("Please enter host port for mysql [default: 3306] ") || 3306);
        }
        else {
            try {
                fs_1.default.rmSync(path_1.default.join(projectDir, "src/datasources/mysql.prisma"));
                fs_1.default.rmSync(path_1.default.join(projectDir, "src/functions/com/biz/ds/cross_db_join.yaml"));
                fs_1.default.rmSync(path_1.default.join(projectDir, "src/events/cross_db_join.yaml"));
            }
            catch (ex) { }
...
async function changeVersion(version, composeOptions) {
    ...
    (0, replace_in_file_1.replaceInFile)({
        ...
        if (!changedFiles[0].hasChanged) {
            ...
            try {
                await prepareContainers(gs.projectName, version, ".devcontainer", composeOptions, gs.mongodb, gs.postgresql, gs.mysql);
            }
            catch (ex) {
