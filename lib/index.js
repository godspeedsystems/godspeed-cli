...
async function GSInit(projectName) {
    const projectDir = path_1.default.resolve(__dirname, '../', projectName);
    ...
    (0, child_process_1.exec)(`chmod 0600 ${devcontainerDir}/mongo-keyfile`, (error) => {
        if (error) {
    ...
    // Start .devcontainer
    await docker_compose_1.default.upAll({ cwd: devcontainerDir, log: true })
    ...
    const res = await docker_compose_1.default.ps({ cwd: devcontainerDir, log: true });
    if (res.out.includes('mongodb1')) {
        console.log('Creating replica set for mongodb');
        await docker_compose_1.default.exec(`${projectName}_mongodb1`, "bash /scripts/mongodb_rs_init.sh", { cwd: devcontainerDir, log: true });
    }
