...
async function prepareContainers(projectName, gsServiceVersion, devcontainerDir, mongodb, postgresql) {
    ...
    if (mongodb) {
        ...
        await docker_compose_1.default
            ...
            console.log("Error in creating replica set for mongodb:", err.message);
        });
    }
    const res = (0, child_process_1.execSync)(`docker pull adminmindgrep/gs_service:${gsServiceVersion}`);
    let commandOptions = ["--pull", "--no-cache"];
    await docker_compose_1.default
        .buildOne("node", {
        executablePath: 'docker',
        cwd: devcontainerDir,
...
async function GSUpdate() {
    ...
    try {
        ...
        try {
            ...
            await prepareContainers(projectName, gsServiceVersion, devcontainerDir, mongodb, postgresql);
...
async function GSCreate(projectName, options) {
    ...
    try {
        ...
        if (elasticsearch) {
            ...
            try {
                fs_1.default.rmSync(path_1.default.join(projectName, "src/datasources/elasticgraph.yaml"));
...
async function changeVersion(version) {
    ...
    (0, replace_in_file_1.replaceInFile)({
        ...
        if (!changedFiles[0].hasChanged) {
            ...
            try {
                await prepareContainers(gs.projectName, version, ".devcontainer", gs.mongodb, gs.postgresql);
            }
            catch (ex) {
