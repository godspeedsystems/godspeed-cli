...
async function GSCreate(projectName) {
    ...
    if (mongodb) {
        ...
        console.log('Starting "prisma genearte and db push"');
        await docker_compose_1.default.exec(`node`, "for f in src/datasources/*.prisma; do npx prisma generate --schema=$f; done && for f in src/datasources/*.prisma; do npx prisma db push --schema=$f; done", { cwd: devcontainerDir, log: true, composeOptions: ["-p", `${projectName}_devcontainer`] })
            .then(() => { console.log('prisma generate and db push are completed'); }, err => { console.log('Error in "prisma genearte and db push":', err.message); });
        // npm install in gs_service
        console.log('Starting npm install');
        await docker_compose_1.default.exec(`node`, "npm install --prefix /workspace/development/app/gs_service", { cwd: devcontainerDir, log: true, composeOptions: ["-p", `${projectName}_devcontainer`] })
            .then(() => { console.log('npm install completed'); }, err => { console.log('Error in executing "npm install":', err.message); });
    }
    else {
        // Start .devcontainer
